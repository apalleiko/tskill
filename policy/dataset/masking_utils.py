...
def get_enc_causal_masks(max_seq_len, max_num_skills, max_skill_len, num_obs, device='cpu'):
    """
    ...
    enc_tgt_mask = torch.ones(max_num_skills, max_num_skills, device=device).to(torch.bool) # Only allow self attention for single step prediction
...
def get_plan_ar_masks(num_img_feats, max_num_skills, goal_mode, num_obs, device='cpu'):
    ...
    if goal_mode == "image":
        ...
        goal_tokens = 1
    plan_src_len = max_num_skills + goal_tokens # (MNS*(q + img_feats) + goal)
