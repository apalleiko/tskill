...
class TSkillCVAE(nn.Module):
    ...
    def __init__(self, stt_encoder, encoder, decoder, state_dim, action_dim, max_skill_len, z_dim, device, **kwargs):
        ...
        self.pos_scale_factor = nn.Parameter(torch.ones(1) * 0.01)
        ...
        self.enc_action_norm = nn.BatchNorm1d(self.hidden_dim)
        
        self.enc_state_proj = nn.Linear(state_dim, self.hidden_dim)  # project qpos -> hidden
        self.enc_state_norm = nn.BatchNorm1d(self.hidden_dim)
        
        self.enc_image_proj = nn.Linear(32,1) #TODO Hardcoded
        self.enc_image_norm = nn.BatchNorm1d(self.hidden_dim)
        
        self.enc_output_norm = nn.BatchNorm1d(self.hidden_dim)
        ...
        self.dec_input_state_norm = nn.BatchNorm1d(self.hidden_dim)
        self.dec_input_image_norm = nn.BatchNorm1d(self.hidden_dim)
        self.dec_input_z_norm = nn.BatchNorm1d(self.hidden_dim)
        self.dec_output_norm = nn.BatchNorm1d(self.hidden_dim)
    ...
    def forward_encode(self, qpos, actions, img_info, max_num_skills, 
        ...
        if is_training:
            ...
            action_src = self.enc_action_norm(action_src.permute(0, 2, 1)).permute(2, 0, 1) # (seq, bs, hidden_dim)
            self.log_metric(type_embed[0, :, :], "mean_enc_action_type", "mean") # METRIC
            ...
            qpos_src = self.enc_state_norm(qpos_src.permute(0, 2, 1)).permute(2, 0, 1) # (seq, bs, hidden_dim)
            self.log_metric(type_embed[1, :, :], "mean_enc_qpos_type", "mean") # METRIC
            ...
            # img_src = self.enc_image_norm(img_src).permute(2, 0, 1) # !(seq, bs, c=hidden)
            self.log_metric(type_embed[2, :, :], "mean_enc_img_type", "mean") # METRIC
            ...
            enc_src_pe = self.get_pos_table(seq) * self.pos_scale_factor
            ...
            enc_tgt_pe = self.get_pos_table(max_num_skills).permute(1, 0, 2).repeat(1, bs, 1) * self.pos_scale_factor
            ...
            enc_output = self.enc_output_norm(enc_output.permute(1, 2, 0)).permute(2, 0, 1)
    ...
    def skill_decode(self, z_sample, qpos, img_info, 
        ...
        self.log_metric(dec_type_embed[0, :, :], "mean_dec_state_type", "mean") # METRIC
        ...
        # image, only use one image to decode rest of the skills 3 TODO batch norm img src?
        img_src = self.dec_input_image_norm(img_src.permute(0, 2, 1)) # (bs, hidden, h*num_cam*w)
        img_src = img_src.permute(2, 0, 1) # (h*num_cam*w, bs, hidden)
        img_pe = img_pe.permute(1, 0, 2) # sinusoidal skill pe
        self.log_metric(dec_type_embed[1, :, :], "mean_dec_img_type", "mean") # METRIC
        ...
        # skills
        z = self.dec_input_z_norm(z.permute(1, 2, 0)).permute(2, 0, 1) # (skill_seq, bs, hidden_dim)
        z_pe = self.get_pos_table(skill_seq).permute(1, 0, 2).repeat(1, bs, 1) * self.pos_scale_factor
        self.log_metric(dec_type_embed[2, :, :], "mean_dec_z_type", "mean") # METRIC
        self.log_metric(z_pe, "mean_dec_z_pe", "mean") # METRIC
        self.log_metric(z, "mean_dec_z_src", "nonzero_mean") # METRIC
        ...
        dec_tgt_pe = self.get_pos_table(seq).permute(1, 0, 2) * self.pos_scale_factor # (seq, 1, hidden_dim)
        ...
        dec_output = self.decoder(src=dec_src + dec_src_pe, # TODO batchnorm dec output?
                          src_key_padding_mask=src_pad_mask, 
                          src_is_causal=False,
                          src_mask=None,
        ...
        dec_output = self.dec_output_norm(dec_output.permute(1, 2, 0)).permute(2, 0, 1) # (seq, bs, hidden_dim)
